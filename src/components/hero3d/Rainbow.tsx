...
const RainbowMaterial = shaderMaterial(
  ...
  /* glsl */ `
    ...
    vec3 iridescence (float angle, float thickness) {      
      ...
      vec3 hue = mix(
        physhue2rgb(thickness * interf0 + dp + time * speed, thickness * phase0), 
        physhue2rgb(thickness * interf1 + 0.1 + dp + time * speed, thickness * phase1), 
        f
      );      
    ...
    void main() {
      ...
      float w = (uv.x / s + .5) * 300. + 400. + a + time * speed;
      ...
      vec3 co = c / iridescence(uv.x * 0.5 * 3.14159, 1.0 - uv.y + time * speed) / 20.0;      
...
type RainbowComponentProps = RainbowProps & {
