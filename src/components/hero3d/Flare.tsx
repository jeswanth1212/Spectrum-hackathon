...
type FlareInstance = THREE.Object3D & {
  scale: THREE.Vector3;
...
export const Flare = forwardRef<THREE.Group, FlareProps>(({ streak = [8, 20, 1], visible, ...props }, fRef) => {
  ...
  useFrame((state) => {
    ...
    ref.current.children.forEach((instance) => {
      if (instance instanceof THREE.Object3D) {
        const flareInstance = instance as FlareInstance;
        flareInstance.position.x = (Math[flareInstance.scale.x > 1 ? 'sin' : 'cos']((state.clock.elapsedTime * flareInstance.scale.x) / 2) * flareInstance.scale.x) / 8;
        flareInstance.position.y = (Math[flareInstance.scale.x > 1 ? 'cos' : 'atan'](state.clock.elapsedTime * flareInstance.scale.x) * flareInstance.scale.x) / 5;
      }
    });
  });
